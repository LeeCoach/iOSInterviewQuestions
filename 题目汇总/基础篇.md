...
#### 27.Object-C有私有方法吗？私有变量呢？

答: objective-c – 类里面的方法只有两种, 静态方法和实例方法. 这似乎就不是完整的面向对象了,按照OO的原则就是一个对象只暴露有用的东西. 如果没有了私有方法的话, 对于一些小范围的代码重用就不那么顺手了. 在类里面声名一个私有方法

| 1234567 | `@interface?Controller?:?NSObject?{?NSString?*something;?}``+?(void)thisIsAStaticMethod;``–?(void)thisIsAnInstanceMethod;``@end``@interface?Controller?(private)?-``(void)thisIsAPrivateMethod;``@end` |
| ------- | ------------------------------------------------------------ |
|         |                                                              |
...
#### 28.Object-C有多继承吗？没有的话用什么代替？cocoa 中所有的类都是NSObject 的子类
...
#### 36.关键字const有什么含意？修饰类呢?static的作用,用于类呢?还有extern c的作用
...
#### 38. 一个参数既可以是const还可以是volatile吗？ 一个指针可以是volatile 吗？解释为什么。
...
#### 42. sprintf,strcpy,memcpy使用上有什么要注意的地方。
...
#### 43. readwrite，readonly，assign，retain，copy，nonatomic 属性的作用
...
#### **3. 一个tableView是否可以关联两个不同的数据源?你会怎么处理?**
...
#### **5. 给出委托方法的实例，并且说出UITableVIew的Data Source方法**
...
#### 7. ViewController的didReceiveMemoryWarning怎么被调用：

答:[supper didReceiveMemoryWarning];

#### 8. 请简要说明viewDidLoad和viewDidUnload何时调用
...
#### 9. Quatrz 2D的绘图功能的三个核心概念是什么并简述其作用。
...
#### 11. 使用AVAudioPlayer类调用哪个框架、使用步骤？
...
#### 12. Cocoa Touch提供了哪几种Core Animation过渡类型？
...
#### **2. 内存管理的几条原则时什么?按照默认法则.那些关键字生成的对象需要手动释放?在和property结合的时候怎样有效的避免内存泄露?**
...
#### **4. 在应用中可以创建多少autorelease对象，是否有限制?**
...
#### 8.内存管理 Autorelease、retain、copy、assign的set方法和含义？
...
| 12   | `typedef?void(^didFinishBlock)?(NSObject?*ob);``这就声明了一个didFinishBlock类型的block，` |
| ---- | ------------------------------------------------------------ |
|      |                                                              |
...
| 123  | `void?dispatch_async(``dispatch_queue_t?queue,``dispatch_block_t?block);` |
| ---- | ------------------------------------------------------------ |
|      |                                                              |
...
#### 1. 做过的项目是否涉及网络访问功能，使用什么对象完成网络功能?

答：NSURLSession和NSURLConnection

####2. 简单介绍下NSURLConnection类及+ sendSynchronousRequest:returningResponse:error:与– initWithRequest:delegate:两个方法的区别?
...
####6. Core Foundation中提供了哪几种操作Socket的方法？

答： CFNetwork 、 CFSocket 和 BSD Socket 。

#### 7. HTTP协议中，POST和GET的区别是什么？
...
#### **1. 看下面的程序,第一个NSLog会输出什么?这时str的retainCount是多少?第二个和第三个呢? 为什么?**
